...
#------------BAIXANDO OS GRAFICOS DE BARRAS DOS DADOS FILTRADOS---------------------------------------
...
#---------- REPETINDO O MESMO PROCESSO COM OS DADOS NAO FILTRADOS ------------------------------------
...
# histograma - idade
grafico_idade_histograma <- ggplot(dados_filtrados, aes(x = IDADE_CALCULADA, fill = IDADE_GRUPO)) + # IDADE_CALCULADA deve ser a idade calculada em anos
  geom_histogram(bins = nclass.Sturges(dados_filtrados$IDADE_CALCULADA), color = "black", alpha = 0.7) +
  scale_fill_brewer(palette = "Set1") +  
  theme_minimal() +
  labs(title = "Histograma de Mortes por substância psicoativa referente a Idade",
       x = "Idade",
       y = "Frequência")

# Exibir o gráfico
print(grafico_idade_histograma)

#Salvar grafico
ggsave("grafico_idade_histograma.png", grafico_idade_histograma, width = 10, height = 6, dpi = 300)



#variaveis ocupação
valores_unicos <- unique(dados_filtrados$OCUP)
